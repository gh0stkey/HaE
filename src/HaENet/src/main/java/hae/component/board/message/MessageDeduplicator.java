...
public class MessageDeduplicator {
    ...
    private boolean isRequestDuplicate(
        HttpRequestResponse newReq,
        HttpRequestResponse existingReq,
        String newUrl,
        String existingUrl,
        String newComment,
        ...
        try {
            ...
            boolean requestBodyMatch = Arrays.equals(
                newReqBody,
                existingReqBody
            );

            // 响应匹配：剥离Dynamic Header后，比对剩余头部 + body
            String newResStripped =
                stripDynamicHeaders(newReq.response()) +
                "\r\n\r\n" +
                new String(
                    newReq.response().body().getBytes(),
                    StandardCharsets.UTF_8
                );
            String existingResStripped =
                stripDynamicHeaders(existingReq.response()) +
                "\r\n\r\n" +
                new String(
                    existingReq.response().body().getBytes(),
                    StandardCharsets.UTF_8
                );
