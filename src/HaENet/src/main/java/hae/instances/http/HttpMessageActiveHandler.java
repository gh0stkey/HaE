...
public class HttpMessageActiveHandler implements HttpHandler {
    ...
    private void setColorAndCommentList(List<Map<String, String>> result) {
        if (result != null && !result.isEmpty()) {
            ...
            if (result.size() >= 3) {
                fingerprintList.get().add(result.get(2).get("fingerprint"));
            }
        }
    }

    private String combineFingerprints(List<String> fingerprints) {
