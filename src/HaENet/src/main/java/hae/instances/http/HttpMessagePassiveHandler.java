    ...
    public AuditResult passiveAudit(HttpRequestResponse httpRequestResponse) {
        ...
        if (!matches) {
            try {
                ...
                setColorAndCommentList(messageProcessor.processRequest(host, request.url(), request, true, true), colorList, commentList);
                setColorAndCommentList(messageProcessor.processResponse(host, request.url(), response, true, true), colorList, commentList);
